...
public:
    ReconstructionWindow(
        ...
        // File/bag playback replays thousands of frames; default auto-freeze
        // runs DBSCAN on every extract and can crash on large surfaces.
        ...
        while (!is_done_) {
            ...
            try {
                ...
                }
                {
                    ...
                    surface_.pcd = DownsamplePointCloudIfNeeded(
...
protected:
    ...
    void UpdateMainImpl() {
        ...
        while (!is_done_) {
            ...
            if (exit_on_empty_frame_) {
                // Pace file/bag playback (~30 fps) so GPU extract/GUI updates
                // do not run far ahead of real-time capture rates.
