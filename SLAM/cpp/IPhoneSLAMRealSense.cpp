...
// ----------------------------------------------------------------------------
//
// Real-time RGB-D SLAM with iPhone LiDAR via Record3D USB streaming.
//
// Copied from RealTimeSLAMRealSense.cpp and adapted for ARKit pose + LiDAR
// depth (float meters). Original RealSense path is preserved in the sibling
// RealTimeSLAMRealSense project.
//
// Build: record3d_cpp (3rdparty/record3d), BUILD_CUDA_MODULE=ON (recommended)
//
// Prerequisites: Record3D iOS app with USB Streaming enabled, Apple Mobile
// Device Support / iTunes on Windows, USB cable.
//
// Examples:
//   IPhoneSLAMRealSense --device CUDA:0 --profile iphone
//   IPhoneSLAMRealSense --pose_source fused --regions
//   IPhoneSLAMRealSense --pose_source arkit --profile iphone
//
...
namespace iphone_capture = open3d::examples::iphone_capture;
using DisplayState = realtime_slam::RealTimeSLAMWindow::DisplayState;
using CapturedFrame = iphone_capture::CapturedFrame;
...
struct SlamParams {
    ...
    // Record3D LiDAR depth is converted to uint16 millimeters.
    float depth_scale = iphone_capture::IPhoneCapture::kDepthScale;
    int update_interval = 15;
    int odom_iter_coarse = 6;
    int odom_iter_mid = 3;
    int odom_iter_fine = 2;
    // arkit: trust ARKit pose; odom: ignore ARKit; fused: ARKit prior + odom.
    std::string pose_source = "fused";
...
SlamParams GetProfile(const std::string& profile) {
    ...
    if (profile == "iphone") {
        // LiDAR depth is coarse (~256x192); slightly larger voxels, 4 m range.
        p.voxel_size = 0.007f;
        p.block_count = 40000;
        p.estimated_points = 5000000;
        p.depth_max = 4.f;
        p.depth_diff = 0.10f;
        p.update_interval = 12;
        p.regions.min_points = 2500;
        p.regions.stability_frames = 4;
        p.regions.interval = 40;
        return p;
    }
...
void PrintHelp() {
    ...
    utility::LogInfo("    > IPhoneSLAMRealSense [options]");
    utility::LogInfo("");
    utility::LogInfo("iPhone / Record3D input:");
    utility::LogInfo("    USB cable + Record3D app with USB Streaming enabled.");
    utility::LogInfo("    Depth is uint16 mm (depth_scale=1000). RGB downscaled");
    utility::LogInfo("    to LiDAR depth resolution for SLAM.");
    utility::LogInfo("");
    utility::LogInfo("SLAM:");
    utility::LogInfo("    [--device CUDA:0|CPU:0] Compute device (default: CUDA:0).");
    utility::LogInfo("    [--profile low|medium|high|iphone]  Preset (default: iphone).");
    utility::LogInfo("    [--pose_source arkit|odom|fused]  Pose fusion (default: fused).");
    utility::LogInfo("                             arkit = trust ARKit VIO pose");
    utility::LogInfo("                             odom  = RGB-D odometry only");
    utility::LogInfo("                             fused = ARKit prior + odometry");
    utility::LogInfo("    [--update_interval N]   Refresh 3D view every N frames.");
    utility::LogInfo("    [--depth_max M]         Depth truncation in meters.");
    utility::LogInfo("    [--voxel_size M]        TSDF voxel size in meters.");
    utility::LogInfo("    [--estimated_points N]  Extract buffer size.");
    utility::LogInfo("    [--block_count N]       TSDF hash block capacity.");
    utility::LogInfo("");
...
// Align ARKit world pose into the SLAM frame using the first-frame calibration.
core::Tensor AlignArkitToSlam(const core::Tensor& T_slam_from_arkit0,
                              const core::Tensor& T_arkit) {
    return T_slam_from_arkit0.Matmul(T_arkit);
}

void SlamWorker(std::function<CapturedFrame()> capture_frame,
    ...
    CapturedFrame first_cap;
    core::Tensor prev_arkit_pose;
    bool have_prev_arkit = false;
    core::Tensor T_slam_from_arkit0 =
            core::Tensor::Eye(4, core::Dtype::Float64, core::Device("CPU:0"));
    bool have_arkit_calib = false;
    const bool use_arkit_pose = (params.pose_source == "arkit" ||
                                 params.pose_source == "fused");
    const bool arkit_only = (params.pose_source == "arkit");

    for (int attempt = 0; attempt < kMaxEmptyCaptureRetries; ++attempt) {
        first_cap = capture_frame();
        first = first_cap.rgbd;
        if (!first.IsEmpty()) {
            if (attempt > 0) {
                utility::LogInfo(
    ...
    if (first.IsEmpty()) {
        utility::LogWarning(
                ...
                "Check that Record3D USB Streaming is running on the iPhone.",
                kMaxEmptyCaptureRetries);
        state.slam_finished.store(true);
        return;
    }
    if (first_cap.has_pose) {
        prev_arkit_pose = first_cap.pose_opencv.Contiguous();
        have_prev_arkit = true;
        // SLAM world starts at identity; calibrate ARKit world -> SLAM.
        T_slam_from_arkit0 = iphone_capture::InvertPose(prev_arkit_pose);
        have_arkit_calib = true;
        utility::LogInfo("ARKit pose calibration locked (pose_source={}).",
                         params.pose_source);
    } else if (use_arkit_pose) {
        utility::LogWarning(
                "First frame has no ARKit pose; falling back to odometry.");
    }
    ...
    while (!state.request_stop.load()) {
        ...
        try {
        CapturedFrame cap =
                (frame_id == 0) ? first_cap : capture_frame();
        t::geometry::RGBDImage rgbd = cap.rgbd;
        if (rgbd.IsEmpty()) {
            if (frame_id == 0) {
        ...
        // Apply ARKit pose prior / absolute reseed before model tracking.
        bool arkit_prior_applied = false;
        if (use_arkit_pose && cap.has_pose && have_arkit_calib &&
            frame_id > 0) {
            const core::Tensor T_arkit = cap.pose_opencv.Contiguous();
            const core::Tensor T_arkit_in_slam =
                    AlignArkitToSlam(T_slam_from_arkit0, T_arkit);
            if (arkit_only) {
                T_frame_to_model = T_arkit_in_slam.Contiguous();
                arkit_prior_applied = true;
            } else if (params.pose_source == "fused") {
                if (tracking_was_unstable &&
                    consecutive_tracking_failures > kLostTrackingThreshold) {
                    // Lost: reseed absolute pose from ARKit (faster than
                    // waiting for global reloc alone).
                    T_frame_to_model = T_arkit_in_slam.Contiguous();
                    arkit_prior_applied = true;
                    if (frame_id % 15 == 0) {
                        utility::LogInfo(
                                "ARKit reseed while lost at frame {}.",
                                frame_id);
                    }
                } else if (have_prev_arkit) {
                    // Healthy: inject frame-to-frame ARKit delta as prior.
                    const core::Tensor delta =
                            iphone_capture::RelativePose(prev_arkit_pose,
                                                         T_arkit);
                    T_frame_to_model =
                            T_frame_to_model.Matmul(delta).Contiguous();
                    arkit_prior_applied = true;
                }
            }
            prev_arkit_pose = T_arkit;
            have_prev_arkit = true;
        } else if (cap.has_pose) {
            prev_arkit_pose = cap.pose_opencv.Contiguous();
            have_prev_arkit = true;
        }
        ...
        if (arkit_only && frame_id > 0 && arkit_prior_applied) {
            // Trust ARKit VIO pose; skip RGB-D odometry refinement.
        ...
        } else if (frame_id > 0 && !prefer_f2f_bridge) {
            {
                std::lock_guard<std::mutex> model_lock(runtime.model_mutex);
                model.UpdateFramePose(frame_id, T_frame_to_model);
                model.SynthesizeModelFrame(
        ...
        if (frame_id > 0 && tracking_was_unstable && !prev_rgbd.IsEmpty() &&
            tracking_tier != TrackingTier::kStrong && !arkit_only) {
            try {
                auto f2f_result =
...
int main(int argc, char* argv[]) {
    ...
    if (argc <= 1) {
        utility::LogInfo(
                "No options given — starting iPhone SLAM with defaults "
                "(CUDA:0, iphone profile, pose_source=fused). Use --help.");
    }
    ...
    const std::string profile = utility::GetProgramOptionAsString(
            argc, argv, "--profile", "iphone");
    if (profile != "low" && profile != "medium" && profile != "high" &&
        profile != "iphone") {
        utility::LogError(
                "Unknown --profile '{}'. Use low, medium, high, or iphone.",
                profile);
        return 1;
    }

    SlamParams params = GetProfile(profile);
    params.pose_source = utility::GetProgramOptionAsString(
            argc, argv, "--pose_source", params.pose_source);
    if (params.pose_source != "arkit" && params.pose_source != "odom" &&
        params.pose_source != "fused") {
        utility::LogError(
                "Unknown --pose_source '{}'. Use arkit, odom, or fused.",
                params.pose_source);
        return 1;
    }
    ...
    if (utility::ProgramOptionExists(argc, argv, "--region_dir")) {
        params.regions.output_dir = utility::GetProgramOptionAsString(
                argc, argv, "--region_dir", params.regions.output_dir);
    }
    ...
    if (utility::ProgramOptionExists(argc, argv, "--region_max_hole_loops")) {
    ...
    utility::LogInfo(
            "SLAM profile: {} (voxel {:.4f} m, depth_max {:.1f} m, blocks {}, "
            "est. points {}, pose_source={})",
            profile, params.voxel_size, params.depth_max, params.block_count,
            params.estimated_points, params.pose_source);
    if (runtime.reloc_config.enabled) {
        utility::LogInfo(
    ...
    iphone_capture::IPhoneCapture iphone;
    if (!iphone.Start()) {
        utility::LogWarning(
                "Failed to start Record3D USB capture. Open Record3D on the "
                "iPhone, enable USB Streaming, press Record, then retry.");
        return 1;
    }

    CapturedFrame warmup;
    for (int attempt = 0; attempt < 60; ++attempt) {
        warmup = iphone.CaptureFrame(500);
        if (!warmup.rgbd.IsEmpty() && warmup.width > 0) {
            break;
        }
        utility::LogInfo(
                "Waiting for first iPhone frame... ({}/60). Start USB "
                "streaming in Record3D if needed.",
                attempt + 1);
    }
    if (warmup.rgbd.IsEmpty() || warmup.width <= 0) {
        utility::LogWarning(
                "Timed out waiting for iPhone RGB-D frames. Ensure Record3D "
                "USB Streaming is active.");
        iphone.Stop();
        return 1;
    }

    params.depth_scale = iphone_capture::IPhoneCapture::kDepthScale;
    Eigen::Matrix3d K = Eigen::Matrix3d::Identity();
    K(0, 0) = warmup.fx;
    K(1, 1) = warmup.fy;
    K(0, 2) = warmup.cx;
    K(1, 2) = warmup.cy;
    core::Tensor intrinsic = core::eigen_converter::EigenMatrixToTensor(K);
    camera::PinholeCameraIntrinsic cam_intrinsic(
            warmup.width, warmup.height, warmup.fx, warmup.fy, warmup.cx,
            warmup.cy);
    utility::LogInfo(
            "iPhone stream ready: {}x{}, fx={:.1f} fy={:.1f} cx={:.1f} "
            "cy={:.1f}, depth_scale={:.1f}",
            warmup.width, warmup.height, warmup.fx, warmup.fy, warmup.cx,
            warmup.cy, params.depth_scale);

    std::mutex warmup_mutex;
    bool warmup_pending = true;
    CapturedFrame warmup_held = std::move(warmup);

    std::function<CapturedFrame()> capture_frame =
            [&iphone, &warmup_mutex, &warmup_pending,
             &warmup_held]() -> CapturedFrame {
                {
                ...
                return iphone.CaptureFrame(500);
            };
    ...
    app.Initialize();
    const auto mono = app.AddFont(visualization::gui::FontDescription(
